...
export default function Home({ onAddToCart }: { onAddToCart: (p: Product) => void }) {
  ...
  const featuredCategories = [
    { name: t('home.categories.food', 'Alimentaire'), icon: '🥫', id: 'alimentaire' },
    { name: t('home.categories.fresh', 'Produits Frais'), icon: '🥩', id: 'frais' },
    { name: t('home.categories.vegetables', 'Fruits & Légumes'), icon: '🥬', id: 'fruits-legumes' },
    { name: t('home.categories.frozen', 'Surgelés'), icon: '❄️', id: 'surgeles' },
  ];
  ...
  return (
    <div className="space-y-12 pb-20">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              ...
              <button className="px-8 py-4 bg-white/10 backdrop-blur-md text-white border border-white/20 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-white/20 transition-all">
                {t('home.hero.stores', 'Nos magasins')}
              </button>
            </div>
          </motion.div>
        </div>
      ...
      <section className="container mx-auto px-4">
        <h2 className="text-2xl font-bold mb-8 text-app-text">{t('home.categories.title', 'Rayons populaires')}</h2>
      ...
      <section className="container mx-auto px-4">
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-3xl font-black text-app-text tracking-tight">{t('home.products.title', 'En ce moment')}</h2>
          <Link to="/products" className="text-brand-blue font-bold flex items-center gap-2 hover:underline">
            {t('home.products.view_all', 'Voir tout')}
            <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="p-8 bg-card-bg rounded-[2.5rem] border border-border-subtle shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
                  ...
                  <h3 className="text-lg font-black text-app-text mb-3 uppercase tracking-tight">{t('home.mission')}</h3>
                  <p className="text-sm text-gray-400 font-medium leading-relaxed">
                    {t('home.mission_text', 'Offrir une plateforme de commerce fluide, sécurisée et ultra-rapide pour tous les Gabonais, partout et tout le temps.')}
                  </p>
                </div>
                ...
                <div className="p-8 bg-card-bg rounded-[2.5rem] border border-border-subtle shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
                  ...
                  <h3 className="text-lg font-black text-app-text mb-3 uppercase tracking-tight">{t('home.values_title', 'Nos Valeurs')}</h3>
                  <p className="text-sm text-gray-400 font-medium leading-relaxed">
                    {t('home.values_text', 'Transparence totale, respect des données utilisateurs et excellence dans le service client sont au cœur de notre ADN.')}
                  </p>
                </div>
              </div>
            </div>
                ...
                <div className="absolute -bottom-8 -right-8 bg-white p-8 rounded-[2.5rem] shadow-2xl border border-gray-100 hidden sm:block">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none">{t('home.passionate', 'Passionné')}</p>
                    </div>
                  </div>
                </div>
              </motion.div>
